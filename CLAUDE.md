...
1. Implement `src/grpo/train.py` using TRL `GRPOTrainer`:
   ...
   - `learning_rate=2e-5` (was 5e-6; raised after 3-seed null — see Progress Log 2026-06-12), `gradient_accumulation_steps=4`, `beta=0.04` (KL).
   - Train subset: 500 prompts of GSM8K `main/train`. Eval slice: **100** prompts of `main/test` every 25 steps (was 50; SE 6.9→4.9 pts).
...
- **LR=2e-5** (raised from 5e-6 — full-param-inherited LR moved the LoRA policy only ~0.01 nats in 100 steps; see Progress Log 2026-06-12), gradient_accumulation_steps=4, **num_generations=4**
...
```
2026-06-13  phase-1  3-seed re-run @ LR 2e-5 / eval 100 complete. Gate (literal) STILL not met: greedy held-out pass@1 0.407±0.047, mean Δ +0.7 pts (need ≥+2). BUT the cause is now identified and is a real result, not an instrument problem: GRPO is SHARPENING, not expanding. Sampled train accuracy rose +15.0/+9.1/+1.9 pts, policy entropy fell ~30% every seed (0.63→0.43 etc.), format saturated to ~0.9, KL 0.017 (≈4× the LR-5e-6 run, exactly as a 4× LR predicts) — yet greedy held-out is flat because greedy already takes the argmax of a distribution GRPO is merely concentrating. This is the Yue et al. (2504.13837) RLVR-boundary phenomenon, already on the project stretch list (§2/§14) and central to H3/pass@k. Step-0 eval identical 0.400 across seeds = clean consistency check (LoRA=identity at init). Results: results/tables/01_grpo_toy.md (LR-2e-5 now primary; 256/5e-6 attempts preserved + documented). Median train FLOPs 2.857e15 = candidate C_toy. Two honest gate-fixes exhausted (512 tokens, 4× LR); both validated the PIPELINE (the toy phase's actual purpose per §1) while showing GRPO doesn't lift greedy pass@1 at 100-step/500-prompt scale. DECISION PENDING with user (phase-structure deviation per §12): (a) proceed to Phase 2 ES with gate reframed as 'pipeline validated + sharpening characterized', do the real ES-vs-GRPO comparison at C_toy in Phase 3 on the same held-out slice; (b) add a cheap toy pass@k measurement first to quantify the sharpening (directly tests H3); (c) keep pushing GRPO greedy (more steps/prompts — diminishing returns, costs §3 budget). Recommend (a)+(b).
2026-06-12  plan-update  Gate-fix decision (user-approved): LR 5e-6→2e-5 (4×; §10 deviation logged — 5e-6 was inherited from full-param GRPO configs and moved the LoRA-only policy ~0.01 nats in 100 steps, undetectable) + eval slice 50→100 items (SE 6.9→4.9 pts). Same 100 steps/3 seeds; projected ~3.8h/seed (within §3). LR-5e-6 runs preserved at runs/*_lr5e-6. Risk watch during re-runs: KL growth and reward collapse per §8 (raise β / restart rules apply). Smoke config LR matched (toy↔smoke science-param equality is test-enforced). Re-launching 3 seeds sequentially.
2026-06-12  phase-1  3-seed toy GRPO @512 complete; **gate NOT met**. Final eval pass@1 0.380±0.040 (s0 +4, s1 −4, s2 0; mean Δ +0.0 vs required ≥+2). BUT training-distribution accuracy rose in ALL 3 seeds (+2.5/+5.0/+8.4 pts) with format adoption, zero collapses, KL max 0.011. Diagnosis: dose homeopathic — LR 5e-6 inherited from full-param configs is tiny for LoRA-only training, β=0.04 leashes further; total policy displacement ~0.01 nats is invisible to a 50-item eval (SE≈6.9 pts; all per-seed deltas within 1 SE). Results table (gate-fail recorded honestly): results/tables/01_grpo_toy.md. Median train FLOPs 2.973e15 = candidate C_toy. Wall ~3.05h/seed (≤4h ✓). Decision pending with user: raise LR (4×, logged §10 deviation) + eval_n 100, vs longer horizon (needs §3 waiver), vs accept-null-and-proceed to Phase 2.
